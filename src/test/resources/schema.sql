...
CREATE TABLE IF NOT EXISTS tf_ticket (
    ...
    response_sla_status VARCHAR(32) NOT NULL DEFAULT 'NORMAL',
    resolution_sla_status VARCHAR(32) NOT NULL DEFAULT 'NORMAL',
...
VALUES
    ...
    ('audit:read',       '查看审计日志',   '审计', '查看本租户的登录与关键操作审计日志');

-- 通用审计日志（V17）。tenant_id 允许为空（登录时租户编码本身就是错的），
-- 所以刻意不加指向 tf_tenant 的外键：审计的意义是"记录发生过的事"，
-- 不该因为被审计对象将来被删掉就写不进去。
